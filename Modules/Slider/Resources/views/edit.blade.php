...
@section ('title', __('slider::labels.backend.slider.management') . ' | ' . __('slider::labels.backend.slider.edit'))
...
@section('content')
{{ html()->modelForm($slider, 'PATCH', route('admin.slider.update', $slider->id))->class('form-horizontal')->attribute('enctype', 'multipart/form-data')->open() }}
    <div class="card">
        <div class="card-body">
            <div class="row">
                <div class="col-sm-5">
                    <h4 class="card-title mb-0">
                        ...
                        <small class="text-muted">{{ __('slider::labels.backend.slider.edit') }}</small>
                    </h4>
                </div><!--col-->
            </div><!--row-->
            ...
            <div class="row mt-4 mb-4">
                <div class="col">
                        ...
                        <div class="col-md-10">
                            <select name="type" id="type" class="form-control select2" required="required">
                                ...
                                @foreach ($sliderType as $id => $name)
                                    <option value="{{ $id }}" {{ $slider->type == $id ? 'selected' : ''  }} name="type">{{ $name }}</option>
                                @endforeach
                            </select>
                        </div><!--col-->
                    </div><!--form-group-->
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-10">
                            @if ($slider->photo)
                                <img src="{{ asset('uploads/' . $slider->photo) }}" alt="Slider Photo" class="img-thumbnail" style="max-width: 200px; max-height: 200px;">
                            @else
                                <span>No Image</span>
                            @endif
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-10">
                            <label class="switch">
                                <input type="checkbox" id="active" name="active" {{ old('active', $slider->active) ? 'checked' : '' }}>
