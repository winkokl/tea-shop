...
Padesar = function(){
    ...
    var handleShowPass = function(){
		jQuery('.show-pass').on('click',function(){
			...
			if(jQuery('#password, .password').attr('type') == 'password'){
				jQuery('#password, .password').attr('type','text');
			}else if(jQuery('#password, .password').attr('type') == 'text'){
				jQuery('#password, .password').attr('type','password');
			}
		});
	}
    ...
    var handleShowPassConfirm = function(){
        jQuery('.show-pass-confirm').on('click',function(){
			jQuery(this).toggleClass('active');
			if(jQuery('#password_confirmation, .password_confirmation').attr('type') == 'password'){
				jQuery('#password_confirmation, .password_confirmation').attr('type','text');
			}else if(jQuery('#password_confirmation, .password_confirmation').attr('type') == 'text'){
				jQuery('#password_confirmation, .password_confirmation').attr('type','password');
			}
		});
    }
	...
	var handleWizardSteps = function (){
		if(jQuery('#DZWizardSteps').length > 0){
			$("#DZWizardSteps").steps({
				...
				autoFocus: true,
				onFinishing: function (event, currentIndex) { window.location.href = "payment-confirm" },
			});
		}
	}
